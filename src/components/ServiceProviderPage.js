...
import { useParams, useNavigate } from 'react-router-dom'; // Ensure useNavigate is imported from react-router-dom
    ...
    const fetchProviders = async () => {
      ...
      try {
        ...
        const filteredProviders = response.data.filter(
          (provider) => provider.city.toLowerCase() === selectedCity.toLowerCase()
        );
      ...
      <div className="container mx-auto mt-8 px-4">
        ...
        {sortedProviders.length === 0 ? (
          <p className="text-center text-gray-500">
            No providers available for {category || ctiveCategory} in {selectedCity}.
          </p>
        ) : (
            ...
            {sortedProviders.map((provider) => (
              <div key={provider.sid} className="bg-white p-6 rounded-lg shadow-md">
                <div className="flex items-start mb-4">
                  ...
                  <div>
                    <h2 className="text-2xl font-bold text-blue-600">name:{provider.name}</h2>
                    <p className="mt-1">Rating: {provider.rating} / 5</p>
                    <p>Experience: {provider.experience}</p>
                    <p>City: {provider.city}</p>
                    <p>Service: {provider.serviceType}</p> {/* Displaying the service type */}
                  </div>
                </div>
                <div className="mb-4">
